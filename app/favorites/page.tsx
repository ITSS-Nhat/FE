...
export default function FavoritesPage() {
  ...
  useEffect(() => {
    // Delete items that were marked for deletion before fetching new data
    deleteMarkedItems()
    fetchFavorites()
  }, [])
  ...
  const deleteMarkedItems = async () => {
    ...
    console.log('Deleting marked items:', Array.from(markedForDeletion))
    
    // Delete each marked item from the database
    for (const favoriteId of markedForDeletion) {
      try {
        await favoriteApi.removeFavorite(favoriteId)
        console.log(`Successfully deleted favorite ${favoriteId}`)
      } catch (err) {
        console.error(`Failed to delete favorite ${favoriteId}:`, err)
      }
    }
      ...
      if (response.status === 'success') {
        if (response.data && response.data.length > 0) {
          ...
          const favoritesWithLikes = response.data.map((item: any) => {
            const likeCount = item.rate ? Math.floor(item.rate * 10000) + Math.floor(Math.random() * 5000) : Math.floor(Math.random() * 50000) + 1000;
            return {
              ...item,
          ...
          .filter((item: any) => !markedForDeletion.has(item.id))
  ...
  const toggleFavorite = async (favoriteId: number) => {
    try {
      // Find the current item
      const currentItem = favorites.find(item => item.id === favoriteId)
      if (!currentItem) return
      
      if (currentItem.isFavorited) {
        // Don't call API yet - just mark for deletion locally
        setMarkedForDeletion(prev => new Set(prev).add(favoriteId))
        
        // Update local state: unfavorite and decrease count
        ...
        showToast.success('お気に入りから削除しました (ページ更新時に確定)')
      } else {
        // Re-favorite: Just undo the deletion (remove from marked for deletion)
        ...
        // Update local state: re-favorite and restore count
        setFavorites(prev => prev.map(item => 
          item.id === favoriteId 
            ? { 
                ...item, 
                isFavorited: true, 
                likes: item.originalLikes // Restore original count
              }
            : item
        ))
        
        showToast.success('削除をキャンセルしました')
      }
    } catch (err) {
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-yellow-50 hover:shadow-md cursor-pointer transition-all"
                      title={dish.isFavorited ? "お気に入りから削除" : "お気に入りに戻す"}
                    >
                      <Heart 
